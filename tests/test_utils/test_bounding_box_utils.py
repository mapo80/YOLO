...
class TestBoxMatcherLargeBoxes:
    ...
    def test_anchor_center_inside_logic(self, box_matcher):
        ...
        NOTE: Uses EXCLUSIVE bounds (aligned with yolov9-official).
        Anchors exactly on the boundary are NOT considered inside.
        """
        ...
        eps = 1e-9  # Same eps used in get_valid_matrix

        for i, (ax, ay) in enumerate(anchor_grid):
            # EXCLUSIVE bounds: anchor must be STRICTLY inside (not on boundary)
            # This matches yolov9-official behavior: bbox_deltas.amin(3).gt_(eps)
            is_inside = (ax > 100 + eps) and (ax < 200 - eps) and (ay > 100 + eps) and (ay < 200 - eps)
            assert valid_matrix[0, 0, i].item() == is_inside, (
                f"Anchor {i} at ({ax}, {ay}) should be "
                f"{'strictly inside' if is_inside else 'outside or on boundary of'} box [100, 100, 200, 200]"
            )

    def test_exclusive_bounds_alignment_with_yolov9(self, box_matcher):
        """
        Test that get_valid_matrix uses EXCLUSIVE bounds (aligned with yolov9-official).

        yolov9-official uses: bbox_deltas.amin(3).gt_(eps)
        This means anchors EXACTLY on the boundary are excluded.
        """
        # Create a box where some anchors are exactly on the boundary
        # With stride=8, anchors are at 4, 12, 20, 28, ... (stride/2 offset)
        # Box [12, 12, 28, 28] has anchors at (12, 12), (20, 20), (28, 28) on boundaries
        target_bbox = torch.tensor([[[12.0, 12.0, 28.0, 28.0]]])

        valid_matrix = box_matcher.get_valid_matrix(target_bbox)
        anchor_grid = box_matcher.vec2box.anchor_grid

        # Find anchors exactly on boundaries
        boundary_anchors = []
        inside_anchors = []
        for i, (ax, ay) in enumerate(anchor_grid):
            on_boundary = (ax == 12 or ax == 28 or ay == 12 or ay == 28)
            strictly_inside = (12 < ax < 28) and (12 < ay < 28)
            if on_boundary and not strictly_inside:
                boundary_anchors.append(i)
            if strictly_inside:
                inside_anchors.append(i)

        # Boundary anchors should be EXCLUDED (exclusive bounds)
        for i in boundary_anchors:
            ax, ay = anchor_grid[i]
            assert not valid_matrix[0, 0, i].item(), (
                f"Anchor {i} at ({ax}, {ay}) is on boundary and should be EXCLUDED"
            )

        # Strictly inside anchors should be INCLUDED
        for i in inside_anchors:
            ax, ay = anchor_grid[i]
            assert valid_matrix[0, 0, i].item(), (
                f"Anchor {i} at ({ax}, {ay}) is strictly inside and should be INCLUDED"
            )


# =============================================================================
# A2: Soft Target Normalization Tests
# =============================================================================

class TestSoftTargetNormalization:
    """
    Tests for A2 fix: soft target normalization aligned with yolov9-official.

    yolov9-official uses: norm_align_metric = (align_metric * pos_overlaps / pos_align_metrics).amax(-2)
    ...
    @pytest.fixture
    def box_matcher_a2(self, create_vec2box_mock):
        """Create a BoxMatcher for A2 testing."""
        from yolo.utils.bounding_box_utils import BoxMatcher
        from omegaconf import OmegaConf

        vec2box = create_vec2box_mock(image_size=(320, 320), strides=[8, 16, 32])
        # BoxMatcher config needs: topk, factor (with iou/cls weights), and iou type
        cfg = OmegaConf.create({
            "topk": 10,
            "iou": "ciou",
    ...
    def test_soft_targets_use_amax_across_targets(self, box_matcher_a2):
        """
        Test that soft target normalization uses .amax() across targets dimension.

        This is the key A2 fix: yolov9 takes the MAX across all targets for each anchor,
        not a gather/select of a specific target's value.
        """
        import inspect
        from yolo.utils.bounding_box_utils import BoxMatcher

        source = inspect.getsource(BoxMatcher)

        # A2 fix: should use .amax(dim=1) or .amax(-2) for max across targets
        assert "amax" in source, "BoxMatcher should use amax for soft target normalization"
        assert "pos_align_metrics" in source, "BoxMatcher should compute pos_align_metrics"
        assert "pos_overlaps" in source, "BoxMatcher should compute pos_overlaps"

    def test_soft_targets_are_in_valid_range(self, box_matcher_a2):
        """
        Test that soft targets are in valid range [0, 1].
        """
        # Create mock predictions
        batch_size = 2
        num_anchors = box_matcher_a2.vec2box.anchor_grid.shape[0]
        num_classes = 13

        # Mock classifier predictions (logits)
        pred_cls = torch.randn(batch_size, num_anchors, num_classes)

        # Mock box predictions (in image coordinates)
        pred_box = torch.rand(batch_size, num_anchors, 4) * 320  # Random boxes

        # Targets: [batch_idx (implicit), class, x1, y1, x2, y2]
        # Shape: [batch, max_targets, 5]
        targets = torch.tensor([
            [
                [0, 50, 50, 150, 150],   # Class 0, medium box
                [1, 100, 100, 200, 200], # Class 1, medium box
                [0, 0, 0, 0, 0],         # Padding
            ],
            [
                [2, 10, 10, 310, 310],   # Class 2, large box
                [0, 0, 0, 0, 0],         # Padding
                [0, 0, 0, 0, 0],         # Padding
            ],
        ], dtype=torch.float32)
        ...
        # Call the matcher
        align_targets, valid_masks = box_matcher_a2(targets, (pred_cls, pred_box))

        # align_targets shape: [batch, num_anchors, num_classes + 4]
        # Extract class scores (soft targets)
        soft_targets = align_targets[..., :num_classes]

        # All soft targets should be in [0, 1] range
        assert (soft_targets >= 0).all(), "Soft targets should be >= 0"
        assert (soft_targets <= 1).all(), "Soft targets should be <= 1"

        # For valid anchors, soft targets should have some non-zero values
        valid_soft = soft_targets[valid_masks]
        if valid_soft.numel() > 0:
            assert valid_soft.max() > 0, "Valid anchors should have positive soft targets"

    def test_soft_targets_normalization_formula(self, box_matcher_a2):
        """
        Test that soft target normalization follows the correct formula.

        Formula: norm_metric = (align_metric * pos_overlaps / pos_align_metrics).amax(-2)
        Where:
        - align_metric = cls_score^alpha * iou^beta
        - pos_overlaps = max IoU per target
        - pos_align_metrics = max align_metric per target

        This test verifies the code structure contains the correct normalization approach.
        """
        import inspect
        from yolo.utils.bounding_box_utils import BoxMatcher
        ...
        # A2 fix: should compute pos_align_metrics and pos_overlaps
        assert "pos_align_metrics" in source, (
            "BoxMatcher should compute pos_align_metrics for normalization"
        )
        assert "pos_overlaps" in source, (
            "BoxMatcher should compute pos_overlaps for normalization"
        )

        # A2 fix: should use .amax(dim=1) to take max across targets
        # This is the key difference from the old implementation
        assert "amax(dim=1" in source or "amax(-2" in source, (
            "BoxMatcher should use .amax(dim=1) to max across targets (A2 fix)"
        )
